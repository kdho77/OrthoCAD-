            ...
            <div className="flex h-[80vh] w-full max-w-4xl flex-col rounded-xl border border-border bg-panel shadow-2xl">
                ...
                <Tabs defaultValue="tokens" className="flex flex-1 flex-col overflow-hidden">
                    <div className="border-b border-border px-4 py-2">
                        <TabsList>
                            <TabsTrigger value="tokens"><Coins className="mr-1 h-3.5 w-3.5" /> Tokens</TabsTrigger>
                            <TabsTrigger value="licenses"><KeyRound className="mr-1 h-3.5 w-3.5" /> Licenses</TabsTrigger>
    ...
    return (
        <div className="max-w-md space-y-3">
            <div className="rounded-md border border-border bg-background/50 p-3 text-xs">
                ...
                <Row label="Expires" value={license?.expiresAt ? new Date(license.expiresAt).toLocaleDateString() : "—"} />
            </div>
            <div className="flex gap-2">
                <Button onClick={renew}>Create / Renew (1y)</Button>
                <Button variant="destructive" onClick={revoke} disabled={!license || license.status === "revoked"}>
                    Revoke
                </Button>
            </div>
